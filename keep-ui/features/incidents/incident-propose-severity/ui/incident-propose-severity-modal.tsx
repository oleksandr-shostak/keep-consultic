...
interface IncidentProposeSeverityModalProps {
...
export function IncidentProposeSeverityModal({
  incident,
  isOpen,
  onClose,
}: IncidentProposeSeverityModalProps) {
  const api = useApi();
    ...
    const loadExistingProposal = async () => {
      try {
        const response = await api.get<ProposalResponse>(
          `/incidents/${incident.id}/propose-severity`
        );
        if (cancelled) {
          return;
        }
  ...
  }, [incident?.id, reason, isSubmitting, isDeleting, isLoadingProposal]);
  ...
  const handleSubmit = async () => {
    ...
    if (!incident.id) {
      setErrorMessage("Incident id is missing. Please refresh and retry.");
      return;
    }
    ...
    try {
      ...
      setErrorMessage(null);
      const endpoint = `/incidents/${incident.id}/propose-severity`;
      const payload = {
        proposed_severity: proposedSeverity,
  ...
  const handleDelete = async () => {
    if (!incident.id || !existingProposal || isSubmitting || isDeleting) {
      return;
    }
  ...
  return (
    <Modal
      ...
      beforeTitle={getIncidentName(incident)}
      description="Save this incident as a triage example for the knowledge base."
    >
      <div className="flex flex-col gap-3">
        ...
        <label className="flex flex-col gap-1">
          ...
          <textarea
            ...
            placeholder="Explain why this incident should have the selected severity."
            value={reason}
